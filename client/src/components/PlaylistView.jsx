...
export default function PlaylistView({ playlist, onToast }) {
 ...
 return (
  ...
  exit={{ opacity: 0, y: -10 }}
  className="w-[calc(100%+2rem)] -mx-4 sm:mx-0 sm:w-full glass-panel border-x-0 sm:border-x border-slate-700/80 rounded-none sm:rounded-3xl p-4 sm:p-5 md:p-6 shadow-[0_0_20px_rgba(0,0,0,0.5)] space-y-6 relative z-10"
  >
  ...
  <div className="relative w-20 h-20 md:w-24 md:h-24 rounded-2xl overflow-hidden bg-slate-900 flex-shrink-0 shadow-[0_0_15px_rgba(6,182,212,0.3)] border border-cyan-500/40">
  {playlist.thumbnail ? (
  ...
  <span className="absolute bottom-1.5 left-2 text-[10px] font-black text-cyan-300 px-1.5 py-0.5 rounded border border-cyan-500/30 bg-slate-900/80 font-mono tracking-wider">
  {totalItems} VIDEOS
  </span>
  </div>
  ...
  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[10px] font-black tracking-wider uppercase bg-cyan-500/20 text-cyan-400 border border-cyan-500/30">
  ...
  by {playlist.uploader || 'Creator'}
    ...
    <h2 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-snug line-clamp-2 drop-shadow-[0_0_8px_rgba(255,255,255,0.2)]">
  {playlist.title}
  </h2>
  </div>
  </div>
  ...
  <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-700/60 shadow-inner">
 ...
 <Video size={12} className="text-cyan-400" /> Master Video Quality
 </label>
 <select
 value={masterVideo}
 ...
 className="w-full bg-slate-800 border border-slate-700 text-cyan-50 rounded-xl px-3 py-2 text-xs font-semibold focus:outline-none focus:border-cyan-500/50 cursor-pointer disabled:opacity-50"
 >
 ...
 <div className="flex items-center justify-between text-[9px] text-slate-500 gap-1 pt-0.5">
 ...
 <div className="flex-1 sm:w-56 space-y-1">
 <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5 font-mono">
 <Music size={12} className="text-violet-400" /> Master Audio Quality
 </label>
 <select
 value={masterAudio}
 disabled={batchState.isDownloading}
 ...
 className="w-full bg-slate-800 border border-slate-700 text-violet-50 rounded-xl px-3 py-2 text-xs font-semibold focus:outline-none focus:border-violet-500/50 cursor-pointer disabled:opacity-50"
 >
 ...
 <span className="text-violet-400 font-mono font-bold whitespace-nowrap">
 ...
 <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 px-4 py-2.5 bg-cyan-900/10 border border-cyan-500/20 rounded-xl text-xs shadow-inner">
 <div className="flex items-center gap-2.5 text-cyan-300 font-medium font-mono">
 <Loader2 size={15} className="animate-spin text-cyan-400 flex-shrink-0" />
 <span>
 ...
 <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded border border-cyan-500/30 bg-cyan-500/10 text-cyan-300">
 Max Video: {maxPlaylistResolution.toUpperCase()}
 </span>
 <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded border border-violet-500/30 bg-violet-500/10 text-violet-300">
 Max Audio: {maxPlaylistAudio.toUpperCase()}
 </span>
 </div>
 </div>
 ) : (
 <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl text-xs shadow-inner">
 <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-300 font-medium font-mono">
 ...
 <div className="flex items-center gap-2 self-end sm:self-auto flex-wrap">
 <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-500/30 bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300">
 Top Video: {maxPlaylistResolution.toUpperCase()}
 </span>
 <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded border border-violet-200 dark:border-violet-500/30 bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-300">
 ...
 <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-zinc-50 dark:bg-zinc-800 p-4 rounded-xl border border-zinc-200/50 dark:border-zinc-800">
 <div className="flex items-center gap-3">
 <button
 type="button"
 disabled={batchState.isDownloading}
 onClick={toggleSelectAll}
 className="touch-manipulation flex items-center gap-2 px-3.5 py-2 rounded-lg text-xs font-bold bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 hover:border-indigo-500/50 active:scale-[0.98] transition-all shadow-sm disabled:opacity-50"
 >
 {isAllSelected ? (
 <>
 ...
 <>
 <Square size={16} className="text-zinc-400" /> Select All ({totalItems})
 </>
 )}
 </button>
 ...
 className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg pl-8 pr-3 py-1.5 text-xs text-zinc-800 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
 />
 {searchQuery && (
 <button
 ...
 className="touch-manipulation w-full md:w-auto px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white hover: hover: text-white font-bold rounded-lg text-sm transition-all hover:shadow-lg hover: active:scale-[0.98] flex items-center justify-center gap-2.5 disabled:opacity-50 disabled:pointer-events-none"
 >
 <DownloadCloud size={17} /> Download Selected ({selectedCount})
 {collectiveTotals.totalBytes > 0 && (
 <span className="px-2 py-0.5 rounded-lg bg-white text-indigo-700 dark:text-indigo-700 text-xs font-mono font-bold tracking-tight">
 ~{collectiveTotals.totalFormatted}
 </span>
 )}
 </button>
 ) : (
 <div className="flex items-center gap-3 bg-zinc-900 text-white px-4 py-2 rounded-lg shadow-lg border border-zinc-700">
 ...
 className="touch-manipulation ml-2 px-2.5 py-1 rounded-lg bg-red-500 hover:bg-red-500 text-red-300 hover:text-white text-xs font-bold transition-colors flex items-center gap-1"
 >
 <X size={12} /> Stop
 </button>
 </div>
 ...
 <div
 key={item.id}
 className={`flex flex-col sm:flex-row sm:items-center gap-2.5 sm:gap-3 p-2.5 sm:p-3 rounded-lg transition-all border ${
 isCurrentActive
 ? 'bg-indigo-50 border-indigo-500/40 shadow-sm ring-1 ring-indigo-500/20'
 : isSelected
 ? 'bg-white dark:bg-zinc-800 border-zinc-200/80 dark:border-zinc-700/80 hover:border-zinc-300 dark:hover:border-zinc-600'
 : 'bg-zinc-50 dark:bg-zinc-900 border-zinc-200/40 dark:border-zinc-800/50 opacity-60'
 }`}
 >
 ...
 <Square size={17} className="text-zinc-300 dark:text-zinc-600" />
 )}
 </button>
 ...
 <div className="relative w-20 h-11 rounded-lg overflow-hidden bg-zinc-200 dark:bg-zinc-800 flex-shrink-0">
 <img src={item.thumbnail} alt="" className="w-full h-full object-cover" />
 {item.durationText && (
 <span className="absolute bottom-0.5 right-0.5 text-[8px] font-mono font-bold bg-black text-white px-1 rounded">
 {item.durationText}
 </span>
 )}
 {(() => {
 ...
 {!videoHasAudio && (
 <span className="absolute top-0.5 right-0.5 text-[7px] font-bold px-1 py-px rounded bg-red-500 text-white">
 <VolumeX size={8} />
 </span>
 )}
 </div>
 ...
 className="text-[13px] font-semibold text-zinc-800 dark:text-zinc-100 hover:text-indigo-500 transition-colors line-clamp-1 block"
 title={item.title}
 >
 {item.title}
 </a>
 <div className="flex items-center gap-2 text-[10px] text-zinc-400 overflow-hidden">
 <span className="truncate max-w-[120px]">{item.uploader || playlist.uploader}</span>
 <span className="text-zinc-300 dark:text-zinc-600">·</span>
 <span className="font-medium text-zinc-500 dark:text-zinc-400 truncate">{formatTag}</span>
 ...
 <div className="flex items-center gap-1 bg-indigo-50 dark:bg-indigo-500 text-indigo-700 dark:text-indigo-100 border border-indigo-200 dark:border-indigo-500 px-2 py-1 rounded-lg text-[11px] font-mono font-bold">
 <Loader2 size={12} className="animate-spin" />
 <span>{statusObj.progress || '0%'}</span>
 </div>
 ) : statusObj?.status === 'completed' ? (
 <div className="flex items-center gap-1 text-emerald-700 dark:text-emerald-100 bg-emerald-50 dark:bg-emerald-500 border border-emerald-200 dark:border-emerald-500 px-2 py-1 rounded-lg text-[11px] font-bold whitespace-nowrap">
 <CheckCircle2 size={12} />
 <span>{statusObj.resolvedFormat || 'Done'}</span>
 </div>
 ) : statusObj?.status === 'error' ? (
 <div className="flex items-center gap-1 text-red-500 bg-red-500 px-2 py-1 rounded-lg text-[11px] font-bold">
 <AlertCircle size={12} />
 <span>Failed</span>
 </div>
 ...
 itemOv.video
 ? 'bg-indigo-50 dark:bg-indigo-950 border-indigo-300 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300 font-bold'
 : 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 font-medium'
 }`}
 >
 {(() => {
 ...
 : itemOv.audio
 ? 'bg-purple-50 dark:bg-purple-950 border-purple-300 dark:border-purple-700 text-purple-700 dark:text-purple-300 font-bold'
 : 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 font-medium'
 }`}
 >
 {!videoHasAudio ? (
