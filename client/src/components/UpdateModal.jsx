  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
      <motion.div
        ...
        className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
      >
        {/* Header */}
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {updateInfo.updateAvailable ? (
                  ...
                  <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-slate-500/10 text-slate-600 dark:text-slate-400 border border-slate-500/20">
                    Up to date
                  </span>
                )}
                {/* Bump type badge */}
                {bumpType === 'major' && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-500/20 uppercase tracking-wider">
                    Major Upgrade
                  </span>
                )}
                {bumpType === 'minor' && (
                ...
                {bumpType === 'patch' && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20 uppercase tracking-wider">
                    Patch Update
                  </span>
                )}
                {/* Channel badge */}
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Current: <span className="font-semibold text-slate-700 dark:text-slate-300 font-mono">v{currentVersion}</span> • Target: <span className="font-semibold text-indigo-600 dark:text-indigo-400 font-mono">v{latestVersion}</span>
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-2.5 bg-slate-100/70 dark:bg-slate-800/40 border-b border-slate-200/60 dark:border-slate-800 flex items-center justify-between text-xs">
        ...
        <div className="flex border-b border-slate-200/80 dark:border-slate-800 px-6 bg-slate-50/40 dark:bg-slate-900/30 text-xs font-bold">
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1 text-slate-700 dark:text-slate-300">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-700/60">
                <div>
              ...
              {isElectron ? (
                <div className="p-4 rounded-2xl bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/40 space-y-3">
                  ...
                  {electronState.status === 'downloading' && (
                    <div className="space-y-1">
                      <div className="w-full bg-indigo-200 dark:bg-indigo-900/50 h-2.5 rounded-full overflow-hidden">
                        <div 
                    ...
                    {electronState.status === 'available' && (
                      <>
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs transition-all shadow-md shadow-indigo-600/20 active:scale-95"
                        >
                          <Download size={14} />
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                    ...
                    {electronState.status === 'downloaded' && (
                      <>
                        <button
                          ...
                          disabled={isApplying}
                          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs transition-all shadow-md shadow-emerald-600/20 active:scale-95"
                        >
                ...
                <div className="p-4 rounded-2xl bg-sky-500/5 dark:bg-sky-500/10 border border-sky-500/20 text-xs space-y-2">
                  ...
                  {updateInfo.updateAvailable && (
                    <button
                      onClick={handleSnooze}
                      className="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-[11px] font-semibold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100"
                    >
                      <BellOff size={12} />
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/30 border border-slate-200/80 dark:border-slate-700/60 space-y-2.5">
                <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px]">
                  <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-0.5">
                    <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Publisher</div>
                    <div className="font-semibold text-slate-700 dark:text-slate-200 truncate">AryansDevStudios</div>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-0.5">
                    <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Code Signing</div>
                  ...
                  <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-0.5">
                    <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Integrity Enforced</div>
            ...
            <div className="space-y-4">
              {hasCategorizedNotes ? (
                <div className="space-y-3">
                  ...
                  {categories.fixes.length > 0 && (
                    <div className="p-4 rounded-2xl bg-sky-500/5 dark:bg-sky-500/10 border border-sky-500/20 space-y-2">
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200/80 dark:border-slate-800 text-xs leading-relaxed max-h-72 overflow-y-auto whitespace-pre-wrap font-mono text-slate-600 dark:text-slate-300">
                  {releaseNotes}
                </div>
              )}
              ...
              {hasCategorizedNotes && (
                <details className="text-xs text-slate-400 cursor-pointer">
                  <summary className="font-semibold hover:text-slate-600 dark:hover:text-slate-200">
                    View Complete Raw Markdown Changelog
                  </summary>
                  <div className="mt-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 text-[11px] font-mono whitespace-pre-wrap">
                    {releaseNotes}
                  </div>
                </details>
              )}
